...
abstract class Breakpoint with _$Breakpoint {
  const factory Breakpoint({required final LayoutType layoutType, required final WindowType windowType}) = _Breakpoint;

  const Breakpoint._();

  /// Breakpoint from size.
  factory Breakpoint.fromSize(final Size size) =>
      Breakpoint(layoutType: LayoutType.fromSize(size), windowType: WindowType.fromSize(size));
}
...
enum LayoutType {
  ...
  factory LayoutType._fromWidth(final double width, {final bool isLandscape = false}) => isLandscape
      ? switch (width) {
