...
final class TextBone extends Bone {
  ...
  const TextBone({required final TextStyle style, super.key, super.width})
    : _style = style,
      super(height: null, contextPadding: EdgeInsets.zero, shape: null);

  /// Text Bone from text
  factory TextBone.fromText(final String text, {required final TextStyle style, final Key? key}) {
    ...
    return TextBone(key: key, style: style, width: width);
  }
  ...
  Widget build(final BuildContext context) {
    ...
    return Padding(
      ...
      child: DecoratedBox(
        decoration: const ShapeDecoration(color: BASE_COLOR, shape: StadiumBorder()),
        child: SizedBox(width: _width ?? double.infinity, height: fontSize),
      ),
    );
  }
}
...
final class MultiLineTextBone extends Bone {
  // ignore: public_member_api_docs
  const MultiLineTextBone({required final TextStyle style, super.key, final int lines = 3})
    : _style = style,
      _lines = lines,
  ...
  Widget build(final BuildContext context) => Column(
    ...
    children: <Widget>[
      ...List<TextBone>.generate(
        _lines - 1,
        (final int index) => TextBone(key: Key("MultiLineTextBone_TextBone_${index + 1}"), style: _style),
      ),
      FractionallySizedBox(
        widthFactor: 0.7,
        child: TextBone(key: Key("MultiLineTextBone_TextBone_$_lines"), style: _style),
      ),
    ],
  );
}
