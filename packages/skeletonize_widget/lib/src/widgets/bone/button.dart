...
final class ButtonBone extends Bone {
  ...
  Widget build(final BuildContext context) {
    ...
    final Size? minimumSize = style?.minimumSize?.resolve(<MaterialState>{});
    final Size? fixedSize = style?.fixedSize?.resolve(<MaterialState>{});

    final Size(
      :double width,
      :double height,
    ) = fixedSize ?? minimumSize ?? Size(_width ?? 64, _height ?? 40);
    final OutlinedBorder shape = style?.shape?.resolve(<MaterialState>{}) ?? const StadiumBorder();
...
final class IconButtonBone extends Bone {
  ...
  Widget build(final BuildContext context) {
    ...
    final double iconSize = _iconSize ?? style?.iconSize?.resolve(<MaterialState>{}) ?? 24.0;
    final EdgeInsetsGeometry contentPadding = style?.padding?.resolve(<MaterialState>{}) ?? const EdgeInsets.all(8);
